...
- 경기 선택과 경기 전/경기 후 상태 분기
- 경기 전 직관메이트 창 진입과 메이트 찾기
- 경기 후 경기 summary/오늘 잘한 선수 노출
- 경기 후 하이라이트 메이커 내부 출전 선수 선택
- 경기 후 미출전 선수 흐림 처리와 선택 비활성화
- 경기 후 선택 경기/출전 선수 기반 AI 하이라이트 생성 인터랙션
- 작전판AI 초보/농잘알 해설
...
- 팬 앱 화면 구현
  - 선호팀 전환, 경기 선택, 경기 상태 표시
  - 경기 전 상태에서는 직관메이트 창 CTA와 메이트 찾기 화면 연결
  - 경기 후 상태에서는 경기 summary와 오늘 잘한 선수 카드 표시
  - 경기 후 하이라이트 메이커 내부 선수 선택과 선택 선수 수 표시
  - 경기 후 해당 경기를 뛰지 않은 선수 흐림 처리와 선택 비활성화
  - 경기 후 AI 하이라이트 생성 버튼, 생성 중 상태, 생성 완료 토스트
  - 생성된 클립 카드와 작전판AI 패널 연결
  - 입덕패스 경기 summary/댓글 카드와 직관메이트 모집글 작성 모달
- 구단 대시보드 구현
  - 이번 주 매출 기회 Top 3 카드
...
- 팀별 최근/예정 경기 카드 렌더링
- 경기 카드에 예정/종료 상태 표시
- 예정 경기 선택 시 직관메이트 CTA 표시
- 종료 경기 선택 시 경기 summary와 오늘 잘한 선수 표시
- 종료 경기 선택 이후 하이라이트 메이커 활성화

사용자 메시지:

- 경기 전에는 같이 갈 사람을 먼저 찾고, 경기 후에는 이 경기에서 누가 잘했는지 먼저 알 수 있다.
...
- 경기 전에는 하이라이트보다 같이 갈 사람을 찾는 것이 우선이다.
...
- 팬은 경기 summary를 본 뒤, 이 경기에서 보고 싶은 선수의 장면만 모아볼 수 있다.
...
| targetId | 대상 ID |
| actionType | view, select, save, share, comment, react, click, apply |
| createdAt | 발생 시각 |
...
3. `ai_ops` service layer가 FanAction 집계, 경기 summary, 출전 선수 목록, 입덕패스 댓글 반응, Player Context DB, 경기 일정, 좌석/굿즈 재고, 스폰서 제약 조건을 조회한다.
...
8. 프론트엔드는 저장된 결과를 Event Builder, Sponsor Package Builder, Funnel / ROI, 생성 결과함에 표시한다.
...
- 모바일 앱처럼 보여야 한다.
- 초보 팬이 부담 없이 들어와 경기 전에는 "누구와 갈지", 경기 후에는 "누가 잘했고 무엇을 보면 되는지" 바로 알 수 있어야 한다.
- 카드 기반 정보 구조를 사용한다.
- 팀 컬러를 적극적으로 반영한다.
- 경기 전은 경기 선택 → 직관메이트 찾기 흐름으로 읽혀야 한다.
- 경기 후는 경기 선택 → summary/오늘 잘한 선수 → 하이라이트 메이커 → 작전판AI/입덕패스 흐름으로 읽혀야 한다.
...
- 경기 전: 경기 선택 후 직관메이트 창 진입과 메이트 찾기 시연
- 경기 후: 경기 선택, summary, 오늘 잘한 선수, 하이라이트 메이커, 미출전 선수 비활성화, 댓글 반응 시연
