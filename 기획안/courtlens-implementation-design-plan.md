...
| 제품 표면 | 대상 | 목적 | MVP 상태 |
| --- | --- | --- | --- |
| 팬 앱 | KBL 팬 | 경기 summary, 출전 선수 기반 하이라이트, 쉬운 해설, 직관메이트 제공 | 구현 |
...
- 팬 앱 화면 구현
  - 선호팀 전환, 경기 선택, 경기 summary 표시
  - 오늘 잘한 선수 카드 표시
  - 하이라이트 메이커 내부 선수 선택과 선택 선수 수 표시
...
- 팬 앱 디자인
  - 모바일 우선 레이아웃
  - 팀 컬러 기반 브랜딩
  - 경기 선택, 경기 summary, 하이라이트 메이커, 출전 선수 선택, 작전판AI, 입덕패스, 직관메이트 화면
...
- 팀 컬러 자동 반영
- 팀별 최근/예정 경기 카드 렌더링
- 경기 선택 시 경기 summary 표시
- 오늘 잘한 선수 또는 스타 플레이어 카드 표시
- 경기 선택 이후 하이라이트 메이커 활성화

사용자 메시지:

- 농구를 잘 몰라도 이 경기에서 누가 잘했는지 먼저 알 수 있다.
...
- 모바일 앱처럼 보여야 한다.
- 초보 팬이 부담 없이 들어와 "어떤 경기에서 누가 잘했고 무엇을 보면 되는지" 바로 알 수 있어야 한다.
- 카드 기반 정보 구조를 사용한다.
- 팀 컬러를 적극적으로 반영한다.
- 경기 선택 → summary/오늘 잘한 선수 → 하이라이트 메이커 → 작전판AI/입덕패스 순서가 한 화면 흐름으로 읽혀야 한다.
...
- 경기 선택, summary, 오늘 잘한 선수, 하이라이트 메이커, 미출전 선수 비활성화, 댓글 반응 시연
...
팬 앱은 라이트 팬이 경기에서 누가 잘했는지 이해하고 직관으로 넘어가게 만들고, 구단 대시보드는 Fan Insight, Revenue Playbook, AI Strategy Engine, Event Builder, Sponsor Package, Performance를 통해 B2B SaaS로서의 구매 이유를 만든다.
