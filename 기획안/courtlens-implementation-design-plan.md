...
MVP는 정적 프로토타입이지만, 역할별로 무엇을 맡아야 하는지 분리해두어야 제품화 단계에서 바로 백로그로 전환할 수 있다.
...
- 제품화 전환 시
  - 정적 JS 객체를 API 응답으로 교체
  - React/Next.js와 TypeScript로 컴포넌트화
  - 이벤트 로그 수집용 `trackFanAction()` 같은 공통 함수 추가
...
1. 프론트엔드가 `POST /api/clubs/:teamId/ai/strategy-draft`를 호출한다.
2. 백엔드는 FanAction 집계, Player Context DB, 경기 일정, 좌석/굿즈 재고, 스폰서 제약 조건을 조회한다.
3. `gpt-5.4-nano`가 후보 이벤트와 팬 반응을 1차 점수화한다.
4. `gpt-5.4-mini`가 Fan Voice와 행동 로그를 분류하고 세그먼트 라벨을 만든다.
5. `gpt-5.5`가 Top 3 기회, 이벤트 브리프, 스폰서 제안서, Performance 개선안을 구조화 JSON으로 생성한다.
6. 백엔드는 JSON schema를 검증하고 근거 지표를 붙여 `GeneratedAsset` 또는 `RevenueOpportunity`로 저장한다.
7. 프론트엔드는 저장된 결과를 Event Builder, Sponsor Package Builder, 생성 결과함에 표시한다.
