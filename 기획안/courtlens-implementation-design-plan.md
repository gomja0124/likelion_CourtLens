...
| 제품 표면 | 대상 | 목적 | MVP 상태 |
| --- | --- | --- | --- |
| 팬 앱 | KBL 팬 | 경기 전 직관메이트, 경기 후 summary와 출전 선수 기반 하이라이트 제공 | 구현 |
...
- 하이라이트 카드 선택 시 오른쪽 상세 패널의 작전판AI 설명을 갱신한다.
...
- 생성 결과는 구단 대시보드에서 "작전판AI 열람률이 높은 장면"으로 집계되어 이벤트/스폰서 후보가 된다.
...
- 팬 댓글 리스트
- 댓글 작성/공감/반응 버튼
- 댓글 반응을 Fan Voice와 구단 대시보드 입력값으로 저장
...
3. 팬 앱 요청이면 `ai_ops` service layer가 clip metadata, 경기 summary, 박스스코어, play-by-play, 선수 프로필을 조회한다.
4. `gpt-5.5`가 작전판AI 설명 또는 입덕패스 문구를 구조화 JSON으로 생성한다.
5. 구단 대시보드 요청이면 `ai_ops` service layer가 FanAction 집계, 입덕패스 댓글 반응, Player Context DB, 경기 일정, 좌석/굿즈 재고, 스폰서 제약 조건을 조회한다.
...
- 구단 대시보드 Top 3/Builder/ROI/Fan Voice 시연
...
팬 앱은 경기 전 라이트 팬이 직관메이트를 찾고, 경기 후에는 누가 잘했는지 이해하게 만든다. 구단 대시보드는 Fan Insight, Revenue Playbook, AI Strategy Engine, Event Builder, Sponsor Package, Performance를 통해 B2B SaaS로서의 구매 이유를 만든다.
