...
현재 구현은 프론트엔드 프로토타입에 로그인 백엔드를 얇게 붙인 MVP입니다. KBL 시즌 일정과 LG vs KT 샘플 경기 데이터는 `app.js`에 포함해 데모 흐름을 재현하고, 사용자 계정은 로컬 `.local/users.json`에 저장합니다.
...
별도 패키지 설치 없이 Node 내장 서버로 실행할 수 있습니다.
...
로그인 API는 같은 서버의 `/api/auth/*` 경로로 제공됩니다.
...
├── server/server.js                        # 정적 파일 서버 + 로그인 API
