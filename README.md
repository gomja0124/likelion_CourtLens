...
4. 섹션 안의 `1Q`, `2Q`, `3Q`, `4Q` 버튼으로 quarter를 선택합니다. 현재 실제 클립은 `1Q`, `2Q`, `3Q`, `4Q` 전체에 연결되어 있습니다.
5. 1Q~4Q timeline row를 누르면 원본 경기 영상이 아니라 `q1_cutpoint_segmentss/`, `q2_cutpoint_segmentss/`, `q3_cutpoint_segmentss/`, `q4_cutpoint_segmentss/`에 저장된 잘린 `.mov` 클립이 바로 재생됩니다. OT1 row는 영상 없이 timeline 기록만 보여줍니다.
...
q4_cutpoint_segmentss/                    # Git LFS로 관리되는 4Q 잘린 클립 36개
...
q4_cutpoint_segments_plan.json            # 4Q cut point 기반 클립 생성 계획
...
```bash
ls -lh q1_cutpoint_segmentss q2_cutpoint_segmentss q3_cutpoint_segmentss q4_cutpoint_segmentss
```

각 `.mov` 파일이 수십 MB에서 수백 MB로 보이면 정상입니다. 몇십 바이트 정도로 보이면 LFS 포인터만 받은 상태이므로 `git lfs pull`을 다시 실행합니다.

원본 전체 경기 영상 `video/1Q.mov`, `video/2Q.mov`, `video/3Q.mov`, `video/4Q.mov`는 이 브랜치의 프론트 실행에는 필요하지 않습니다. 프론트는 이미 잘린 `q1_cutpoint_segmentss/*.mov`, `q2_cutpoint_segmentss/*.mov`, `q3_cutpoint_segmentss/*.mov`, `q4_cutpoint_segmentss/*.mov` 파일을 직접 참조합니다.
...
├── q4_cutpoint_segmentss/                  # FIBA 4Q play-by-play 매칭용 잘린 클립, Git LFS
