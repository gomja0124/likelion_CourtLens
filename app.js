...
const FEATURED_FIBA_MATCH = {
  ...
  postGame: {
    ...
    summary: [
      "1Q~4Q play-by-play를 score, defensive rebound, turnover, foul cut point로 나눠 possession 단위 클립으로 연결했습니다.",
...
function formatPbpClock(clock) {
  ...
  const uniqueClock = (value) => {
    const clocks = String(value || "")
      .split(/\s*\/\s*/)
      .filter(Boolean);
    return [...new Set(clocks)][0] || value;
  };
  const firstStart = uniqueClock(rawStart);
  const firstEnd = uniqueClock(rawEnd);
...
function playByPlayMatchTitle(context = currentContext) {
  const match = context?.match || matchById(selectedMatchId);
  if (!match) return "Play-by-Play";
...
function renderTimelineOnlyEvents(events) {
  events.forEach((item) => {
    const team = eventTeam(item.event);
    ...
    eventCard.innerHTML = `
      ...
      <small>${teamLabel(team)} · ${pbpTimelineEventType(item.event)} · 영상 클립 없음</small>
    `;
...
function renderPlayByPlayTimeline() {
  ...
  const timelineOnlyEvents = hasData ? currentPbpTimelineOnlyEvents() : [];
  const clips = hasData && timelineOnlyEvents.length === 0 ? currentPbpClips() : [];

  if (playByPlayPanel) playByPlayPanel.classList.toggle("pbp-unavailable", !hasData);
  if (pbpMatchTitle) pbpMatchTitle.textContent = playByPlayMatchTitle(currentContext);
  if (pbpQuarterPill) pbpQuarterPill.textContent = hasData ? `FIBA ${selectedPbpQuarter}` : "준비 전";
  if (pbpMatchCount) {
  ...
  if (pbpSideLabelItems.length >= 2) {
    pbpSideLabelItems[0].textContent = timelineOnlyEvents.length ? "대한민국 timeline" : "대한민국 하이라이트";
    pbpSideLabelItems[1].textContent = timelineOnlyEvents.length ? "Chinese Taipei timeline" : "Chinese Taipei 하이라이트";
  }
  ...
  if (!hasData) {
    ...
    empty.textContent = `${playByPlayMatchTitle(currentContext)} 경기는 아직 clipped 영상 데이터가 없습니다. 현재 Play-by-Play 데모는 대한민국 남자농구 대표팀의 대한민국 vs Chinese Taipei 경기만 지원합니다.`;
...
pbpTimeline?.addEventListener("click", (event) => {
  ...
  if (!row || !row.dataset.pbpClipId) return;
